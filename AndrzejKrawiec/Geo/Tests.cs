...
7
            ...
            Assert.Equal(file.GetRecord().First(), s);
        }
...
7
            ...
            string[] split = s.Split(default(string[]), StringSplitOptions.RemoveEmptyEntries);
            string nrDzialki = split[0];
            string x = split[1];
            string y = split[2];

            Assert.Equal(nrDzialki, "1.1-800/128");
            Assert.Equal(x, "5544943.71");
            Assert.Equal(y, "6532099.62");
